...
memberMap ={'TUB'   : 'PIPE',       # Tubular or pipe
            'WF '   : 'I',          # Wide flange
...
class SECT:
  ...
  def __init__(self, l):
    ...
    if l[15:18] in memberMap:
      ...
      if False in [s==' '*len(s) for s in (self.AX, self.J, self.IZ, self.IZ)]:
...
class PGRUP:
  ...
  def __init__(self, l, eltype):  
    ...
    self.ID = l[6:9]
    self.NA = l[9]                              # Neutral axis offset
    self.thickness = GetFloat(l[10:16])*1e-2    # Convert from cm to m
    self.sect = l[16]                           # PLATE section - (I=Isotropic, M=Membrane,
                                                # S=Shear Stiff Only, X=x-dir corrugated,
                                                # Y=y-dir corrugated)
    self.E = GetFloat(l[17:23])*1e10            # Modulus, convert from kN/cm^2 to Pa
    self.v = GetFloat(l[23:29])                 # Poisson's ratio
    self.FY = GetFloat(l[29:35])*1e7            # Yield stress, convert from kN/cm^2 to Pa
    self.Zoffset = GetFloat(l[35:41])*1e-2      # Local z-offset, convert from cm to m
...
class PLATE:
  ...
  def __init__(self, l):
    ...
    try:
      ...
      self.E = GetFloat(l[47:54])*1e10          # Modulus - convert from 1000 kN/cm^2 to Pa
      self.v = GetFloat(l[54:59])               # Poisson's ratio
      self.FY = GetFloat(l[59:64])*1e7          # Yield stress - convert from kN/cm^2 to Pa
      self.density = GetFloat(l[69:74])*1e3     # Seastate weight density - convert from t/m^3 to kg/m^3
...
class LCOMB:
  ...
  def AddLoads(self, l):
    ...
    for loadcase_name, loadcase_factor in ( (l[11:15],l[15:21]),(l[21:25],l[25:31]), (l[31:35],l[35:41]), (l[41:45],l[45:51]), (l[51:55],l[55:61]), (l[61:65], l[65:71]) ):
      if loadcase_name.strip():
        self.loadcases.append((loadcase_name.strip(), GetFloat(loadcase_factor)))
...
class JOINT:
  ...
  def Elastic(self, l):
    ...
    self.spring = Spring()
    disp_rates = [GetFloat(x)*1e3 for x in (l[11:18],l[18:25],l[25:32])] # Convert kg/cm to N/m
    rot_rates = [GetFloat(x)*0.1 for x in (l[32:39],l[39:46],l[46:53])] # Convert kg.cm/rad to N.m/rad
    self.spring.rates = disp_rates + rot_rates # [dx, dy, dz, rx, ry, rz]
    self.spring.comments = l[61:69]
    self.spring.joint2 = l[72:76].strip()   # Support coord sys orientation joint. Defines x-axis line.
    self.spring.joint3 = l[76:80].strip()   # Support coord sys orientation joint. Defines x-z plane.
...
if dialog_import:
  if len(sys.argv) > 1:
    ...
    sacs_file = tkFileDialog.askopenfile(parent=root,mode='rb',title='Choose SACS input file')
    if sacs_file == None:
      ...
      if tkMessageBox.askyesno("Additional SACS File", "Import an addition SACS file (e.g. section library)?"):
        sacs_file = tkFileDialog.askopenfile(parent=root,mode='rb',title='Choose SACS input file')
        if sacs_file == None:
          print('No SACS file selected, exiting...')
          os.system('pause')
  ...
  if sacs_file2: file_list.append(sacs_file2)
...
for fname in file_list:
  ...
  for l in f:
    if not l == "JOINT\n" and l[:5] == "JOINT":
      ...
      if l[54:60] == 'ELASTI' and l[6:10] in joints:
        ...
        print('WARNING: A joint spring is defined before its joint is defined! Skipping...\n')
      else:
  ...
  f.close()
print('\nReading complete:\n\t' + str(len(joints)) + ' joints were successfully read\n\t' + str(len(members)) + ' elements (members) were successfully read')
...
for m in members.keys():
  if GetDistance(joints[members[m].jointA], joints[members[m].jointB]) < length_tol:
    ...
    nmap[joints[members[m].jointB].ABQID-1].append('MERGED WITH %s' % joints[members[m].jointA].ABQID)
...
for m in members:
  if abs((joints[members[m].jointA].x)-(joints[members[m].jointB].x)) <= 0.001 and abs((joints[members[m].jointA].y)-(joints[members[m].jointB].y)) <= 0.001:
...
try:
  ...
  print('Error trying to open ' + inp_file + ', check that you have permission to do so. Exiting')
  exit(2)
...
for g in grups:
  ...
  for m in members:
    if members[m].group == g:
      ...
      out.write('%s, %s, %s\n' % (members[m].ABQID, joints[members[m].jointA].ABQID, joints[members[m].jointB].ABQID))
      elnum += 1
for pg in pgrups:      
  for p in plates:
    if plates[p].group == pg:
      ...
      if plates[p].jointD == '':
        ...
        out.write('%s, %s, %s, %s\n' % (plates[p].ABQID, joints[plates[p].jointA].ABQID, joints[plates[p].jointB].ABQID, joints[plates[p].jointC].ABQID))
      else:
        # 4-element shell
  ...
  if g in grups:
    if grups[g].section:
      ...
      try:
        ...
        log.write('ERROR: %s when trying to use section of GROUP: %s' %(sys.exc_value, g))
      if s:
        if s.sect in ('ARBITRARY', 'CHL', 'CON'):
          log.write('Members in group %s are assigned ARBITRARY, CHL or CON section; skipping\n' % g)
        elif s.sect == 'PIPE':
          out.write('*Beam Section, elset=M-%s, section=%s, material=Mtl-Beam\n'  %  (m.replace('.','-'), s.sect)) 
          ...
          if s.C != 0.0 and s.D != 0.0:
            ...
            # Normal pipe
            out.write('%s, %s\n' % (s.A/2.0, s.B)) # outside radius, wall thickness
          assigned = True
        elif s.sect == 'I':
          out.write('*Beam Section, elset=M-%s, section=%s, material=Mtl-Beam\n'  %  (m.replace('.','-'), s.sect)) 
          # l, h, b1, b2, t1, t2, t3
      ...
      if grups[g].OD != 0.0:
        ...
        out.write('*Beam Section, elset=M-%s, section=%s, material=Mtl-Beam\n'  %  (m.replace('.','-'), 'PIPE')) 
        # Abaqus requires input of outside radii, whereas SACS is in OD
        out.write('%s, %s\n' % (grups[g].OD/2, grups[g].thickness))  # outside radius, wall thickness
        assigned = True
      else:
        log.write('Unable to determine section properties for group %s, skipping.' % g)
  else:
    # Member group not found, report as error
    missing_sect_members.append(m)
    log.write('No group definition found for member %s, no section can be assigned!' % m.replace('.','-'))
  if assigned:
    ...
    try:
      missing_sect_members.append(m)
      log.write('Missing section assignment for member %s (Group %s, Section ID %s)\n' % (m.replace('.','-'), members[m].group, grups[g].section))
    except:
...
out.write('**\n** PLATE SECTIONS **\n')
for pg in pgrups:
  out.write('*Shell General Section, elset=PG-%s, material=%s\n' %(pgrups[pg].ID.replace('.','-'), 'Mtl-Plate'))
  out.write('%s\n' % pgrups[pg].thickness)
...
try:
  ...
  print('Error trying to open ' + inp_file + '_nmap.txt, check that you have permission to do so. Exiting')
  exit(2)
out.write('ABQ\t->\tSACS\n')
for n in nmap:
  if len(n) == 3:
...
try:
  ...
  print('Error trying to open ' + inp_file + '_elmap.txt, check that you have permission to do so. Exiting')
  exit(2)
out.write('ABQ\t->\tSACS\n')
elmap = []
for m in members:
...
try:
  ...
  print('Error trying to open ' + inp_file + '_loads.txt, check that you have permission to do so. Exiting')
  exit(2)
out.write('** INDIVIDUAL LOAD CASES **\n**\n')
try:
  for lc in loadcases:
    out.write('**\n** LOAD CASE %s\n** %s\n**\n' % (lc, loadcases[lc].description))
    if loadcases[lc].loads: out.write('*Cload\n')
